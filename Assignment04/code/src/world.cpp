...
double eta = 1.35; //Eta value (Taken from reference mentioned in the report)
double eta_air = 1.0;
...
Color World::shade_ray(Ray& ray, int count)
{
	...
	if(ray.didHit()) {
        if (count <= 10) { //Recursion Limit set to 10
            if(ray.intersected()->isSolid == true){
                ...
                if(n_dot_d < 0){
                    ...
                    //UNCOMMENT THIS CODE FOR BEER'S LAW
                    ...
                    if(!tir_check(d, -1 * n, 1.0/eta)){
                        ...
                        c = dotProduct(t, n);
                        Ray refractedRay(ray.getPosition(), t);
                        double r0 = pow((eta - 1.0), 2)/pow(eta + 1.0, 2); //Schlick's Approximation
                        double r_factor = r0 + (1 - r0)*pow((1-c), 5);
                        return k*(1 - r_factor)*shade_ray(refractedRay, count + 1) + k*r_factor*shade_ray(reflectedRay, count + 1); //Formula for schlick's approximation
                    }
                    else{
